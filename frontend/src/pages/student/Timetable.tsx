...
export const Timetable = () => {
  ...
  useEffect(() => {
    const load = async () => {
      if (!user) return;
      try {
        const res = await timetableApi.getStudentTimetable(user.user_id);
  ...
  return (
    <Box>
      <PageHeader
        title="Timetable"
        description="Your weekly class schedule with rooms and timings."
      />

      {timetable.length === 0 ? (
        <EmptyState
          title="No timetable available"
          description="Your class schedule will appear here once it's configured."
        />
      ) : (
                    ...
                    >
                      ...
                      >
                        <Box sx={{ textAlign: "right" }}>
                          ...
                          <Chip label={entry.room_number} size="small" />
                        </Box>
                      </Box>
                    </Box>
                  ))}
